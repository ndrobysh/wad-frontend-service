  ...
  return (
    <main className={styles.main}>
      <header className={styles.header}>
        ...
        <div><span>{username}</span><button onClick={handleLogout}>Déconnexion</button></div>
      </header>
      ...
      <section className={styles.section}>
        <h2>Invocation</h2>
        <button className={styles.summonBtn} onClick={handleSummon} disabled={loading}>
        ...
        {summonedMonster && (
          <div className={styles.summonResult}>
            ...
            <div className={`${styles.monsterCard} ${styles[summonedMonster.elementType.toLowerCase()]}`}>
              ...
              <p>PV: {summonedMonster.hp} | ATK: {summonedMonster.atk} | DEF: {summonedMonster.def} | VIT: {summonedMonster.vit}</p>
            </div>
          </div>
        )}
      </section>
      ...
      <section className={styles.section}>
        ...
        {monsters.length === 0 ? (
          ...
          <div className={styles.monsterGrid}>
            {monsters.map((m) => (
              <div key={m.id} className={`${styles.monsterCard} ${styles[m.elementType.toLowerCase()]}`}>
                <h4>{m.name}</h4>
                <p>Niv. {m.level} | {m.elementType}</p>
                <p>PV: {m.hp} | ATK: {m.atk} | DEF: {m.def} | VIT: {m.vit}</p>
              </div>
            ))}
          </div>
        )}
